...
const UploadField: FC<IUploadField> = ({
    ...
    return (
        <View
            className={cn('bg-[#232323] w-full border rounded-lg py-2.5 px-4 my-1.5',
                error
                    ? 'border-red'
                    : 'border-transparent')}
            style={style}
        >
            <View className='flex-row justify-between items-center'>
                <View>
                    ...
                    {isNoImage && <Text className='text-white opacity-40 mt-3' numberOfLines={1}>
                        {value}
                    </Text>}
                    {error && <Text className='text-red mt-1'>{error.message}</Text>}
                </View>
                {!isNoImage && <View
                    className='w-24 h-24 rounded-2xl overflow-hidden ml-5'
                >
                    {isLoading
                        ? <Loader/>
                        : !!value && <Image
